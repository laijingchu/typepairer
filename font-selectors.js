...
btnHFontRandomize.addEventListener('click', function () {
	...
	if (selectHFontCategory.value == "any") {
		...
		headerFontSheet.setAttribute("href", `https://fonts.googleapis.com/css?family=${fontDisplay}:100,200,300,400,500,600,700,800,900`);
...
btnBFontRandomize.addEventListener('click', function () {
	...
	if (selectBFontCategory.value == "any") {
		...
		bodyFontSheet.setAttribute("href", `https://fonts.googleapis.com/css?family=${fontDisplay}:100,200,300,400,500,600,700,800,900`);
...
//-----------------DECLARE CONST: FONT NAME LIST---------------------//



const fontDisplays = fonts.map(font => font.display).sort() /*array*/
const fontDisplaysUnsorted = fonts.map(font => font.display)


//-----------------LOAD FONTS INTO SMALL DROPDOWN---------------------//

//header


for (let i = 0; i < fontDisplays.length; i++) {
	let opt = fontDisplays[i]; // each dropdown option is fontDisplays[i]
	let el = document.createElement("option"); //create an option tag
	el.textContent = opt; //the displayed text
	el.value = fontDisplays[i]; //the value tag
	dropdownCurrentHFont.appendChild(el); //adds this option to the dropdown
}
...
selectHFontCategory.addEventListener("input", function () {


	if (selectHFontCategory.value != "any") {

		// clears the dropdown
		document.getElementById("header-font").innerHTML = "";

		// creates an obj array of fonts, filtered by user: serif, sans serif, etc.[{...}, {...}, {...}]
		const filteredFonts = fonts.filter(obj => {
			return obj.type === selectHFontCategory.value
		})

		//this remaps a simpe array with just the font names, and sorts		
		const filteredFontDisplays = filteredFonts.map(filteredFont => filteredFont.display).sort()


		// Populates dropdown with new list //
		for (let i = 0; i < filteredFontDisplays.length; i++) {
			let opt = filteredFontDisplays[i];
			let el = document.createElement("option"); //create an option tag
			el.textContent = opt; //the displayed text
			el.value = filteredFontDisplays[i]; //the value tag
			dropdownCurrentHFont.appendChild(el); //adds this option to the dropdown
		}
	} else {
		// clears the dropdown
...
//user clicks one of the categories
selectBFontCategory.addEventListener("input", function () {


	if (selectBFontCategory.value != "any") {

		// clears the dropdown
		document.getElementById("body-font").innerHTML = "";
		...
		for (let i = 0; i < filteredFontDisplays.length; i++) {
			...
			el.textContent = opt; //the displayed text
			el.value = filteredFontDisplays[i]; //the value tag
			dropdownCurrentBFont.appendChild(el); //adds this option to the dropdown
		}
	} else {
		// clears the dropdown
		document.getElementById("body-font").innerHTML = "";
...
//-----------------USER CLICKS SMALL DROPDOWN---------------------//

dropdownCurrentHFont.addEventListener("input", function () {

	headerFontSheet.setAttribute("href", `https://fonts.googleapis.com/css?family=${dropdownCurrentHFont.value}:100,200,300,400,500,600,700,800,900`);

	allHs.forEach(HTag => {
		HTag.style.fontFamily = dropdownCurrentHFont.value


		// The object's "type" is reflected back in the category selection dropdown


		//find the index number of the font currently inside the dropdown menu within the fonts list.
		const fontIndex = fontDisplaysUnsorted.indexOf(dropdownCurrentHFont.value)


		// pin down object in the array list, and reflect the .type value in the category selection field.
		selectHFontCategory.value = fonts[fontIndex].type
...
dropdownCurrentBFont.addEventListener("input", function () {
	...
	bodyFontSheet.setAttribute("href", `https://fonts.googleapis.com/css?family=${dropdownCurrentBFont.value}:100,200,300,400,500,600,700,800,900`);
	...
	paperPs.forEach(paperP => {
		...
		//find the index number of the font currently inside the dropdown menu within the fonts list.
		const fontIndex = fontDisplaysUnsorted.indexOf(dropdownCurrentBFont.value)


		// pin down object in the array list, and reflect the .type value in the category selection field.
		selectBFontCategory.value = fonts[fontIndex].type
